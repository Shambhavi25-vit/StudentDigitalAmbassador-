# Task 1: Digital Literacy Awareness Infographic

## 📌 Overview
This section contains the visual resources developed during my tenure as a **Student Digital Ambassador**. 

The objective of this task was to design a comprehensive, one-page infographic to help peers navigate the core components of Digital Literacy. By providing actionable steps, this resource aims to bridge the gap between basic internet usage and professional digital fluency. 

> **Note:** The complete written description and reflective analysis for this task are located in the [Main Project Report](../link-to-your-report.md).

---

## 🖼️ Featured Resource
### [The Student's Digital Toolkit: Mastering the Online World]
![Student's Digital Toolkit Infographic](task1pic.png)

*Visual guide created using Canva.*

---

## 🔑 Key Themes Covered
The infographic is structured around three foundational "pillars" of digital citizenship:

1. **Digital Literacy** Transitioning from critical consumption (evaluating sources) to creative production (contributing meaningful content).
2. **Safe Internet Practices** Strategies for enhancing cybersecurity, maintaining vigilance against phishing, and proactive digital footprint management.
3. **Professional Email Etiquette** Mastering essential "Netiquette" standards required for academic and professional communication success.

---

[⬅ Back to Main Repository](../../)
